...
**Description:** Use Lucia v3 only for the _identity layer_ — owning the `users` and credential tables and providing the credential-validation primitives. Sessions are our own: a short-lived EdDSA JWT in an HttpOnly cookie (`__Host-access`, 30 min) plus an opaque refresh token in a second HttpOnly cookie (`__Secure-refresh`, 14 days, stored hashed in `sessions`, rotates on use). Refresh emits a new access token without a DB write to the credentials path.
...
- Cookies: `__Host-access` (Path=/, Secure, HttpOnly, SameSite=Strict), `__Secure-refresh` (Secure, HttpOnly, SameSite=Strict, `Path=/api/auth`). The refresh cookie uses the `__Secure-` rather than `__Host-` prefix because `__Host-` enforces Path=/ at the browser level, which conflicts with our path-scoping defense in depth. `__Secure-` keeps the HTTPS-only guarantee while allowing the narrower path.
